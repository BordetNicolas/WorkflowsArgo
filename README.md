...
Démo sans objet dans le bucket (`simulate_missing_input=true`) :

```bash
argo submit --from workflowtemplate/chaine-directe -n workflows-argo \
  -p input_xml_key=inbox/exemple.xml \
  -p simulate_missing_input=true \
  -p packet_seed=42
```

Avec un XML réel déjà présent dans le bucket MinIO :

```bash
argo submit --from workflowtemplate/chaine-directe -n workflows-argo \
  -p input_xml_key=inbox/exemple.xml \
  -p simulate_missing_input=false
```

| Paramètre | Défaut | Rôle |
|-----------|--------|------|
| `input_xml_key` | *(requis)* | Clé S3 du XML source |
| `packet_seed` | `42` | Graine déterministe du nombre de paquets par canal (0..3) |
| `simulate_missing_input` | `false` | `true` = XML stub sans download MinIO |
